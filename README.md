...
- **King (k/K)**: Your VIP boss with a cross hat! Moves **just 1 square** any way: up, down, left, right, or corner-diagonal. Can't go where danger waits (attacked square).
  Example: Your k at 51 can go 41 (left), 61 (right), 52 (up), 50 (down—nope, off board!), 42/62 (diags). Short and safe—like tip-toeing!

- **Queen (q/Q)**: The powerhouse boss lady! Zooms **any number of squares** straight: up/down/left/right **or diagonal**. Stops at blocks or captures enemy.
  Example: q at 45 (clear path) zips to 40 (down full file) or 05 (left across rank) or 88 (diag up-right). Enemy at 33? Capture to 33!

- **Rooks (r/R)**: Two castle towers! Any distance **up/down/left/right only** (no diags). No jumping blocks.
  Example: r at 11 slides to 110 (up full file) or 19 (right across rank 1).

- **Bishops (b/B)**: Two pointy-hat wizards! Any distance **diagonal only**. Stuck on same color squares forever. No jumps.
  Example: b at 31 goes 42, 53, 64, 75... up to 910! Backward too if clear.

- **Knights (n/N)**: Two horse buddies! Weird L-jump: **2 one way + 1 sideways** (or 1+2). **Jumps over everything**—sneaky!
  Example: n at 21 jumps to 03 (2 left 1 up), 12 (1 left 2 up), 32 (1 right 2 up), 40 (2 right 1 up). Can't go off!

- **Trebuchets (t/T)**: Your new favorite—**two catapults per side**! Medieval boulder-lobbers. Jump **exactly distance 3** (Chebyshev: biggest change in file or rank = exactly 3). Up to **24 spots** in center—like a huge ring around! Skips close squares (no 1-2 away). **Super strong (beats rook)** but get 'em out early—they can't fight neighbors!
  Example: t at 44 blasts to 14 (3 down), 74 (3 up), 01 (3 left 3 down—off? Trim!), 13 (3 left 1 down), **24 spots total central**. From corner t00: 7 spots like 03/12/21/30. Jumps anything!
...
**Black Rank 10**: T0 R1 N2 B3 **Q4 K5** B6 N7 R8 T9.  
**Rank 9**: P0 to P9.  
...
- **Rook Castling**: King hides safe with rook buddy (once per side).  
  ...
  **Queenside (left)**: k51 → 21, r11 → 31 (21/31/41 empty).  
  One turn! King/rook unmoved before.

- **Trebuchet Castling**: Flank fort bomb! King dashes **3 squares** (once/side).  
  **Kingside**: k51 → 81, t91 → 61 (61/71/81 empty).  
  **Queenside**: k51 → 01, t01 → 31 (01/11/21/31 empty).  
  Same rules—epic catapult cover!

- **Pawn Promotion**: Pawn hits enemy back (white rank10, black rank1) → pick **q/r/b/n/t/k**! Boom—instant upgrade.
  Example: p99 to 910 = q! (Even capturing.)

- **Heir Kings**: Promo to **k** = heir prince! Moves like king (1 any).  
  **Super Sneaky**: Not "royal" till your **last king**—treat like normal piece (can capture it)! Check **only** on last king.  
  **Kings Repel**: No king (heir/original) can move/end next to **any** king (own/enemy)—illegal!  
...
- **3x Repeat**: Same setup 3 times (your turn) = tie claim.
- **10-Move Rule**: 10 full turns (20 moves) no captures? Count pieces—**fewer wins**! Equal = tie. (Huge army vs lone k? You lose—smart defense!)
...
- **b**: Super-long diags (09-90!).
- **n**: Holes like 36/46—fork city!
...
- **Pins**: Trap foe—can't move or VIP exposed behind. q/r/b best.
  Setup: Line slider through enemy to their king/q. Ex: r05 pins n57 (K50 behind).
- **Skewers**: VIP front, weak behind—hit VIP, it flees, grab weak.
  Setup: Open file. Ex: q22 skewers R85 (heir k90 behind).
- **X-Rays**: Attack "through" pieces (your pawns ok).
  Setup: r behind p-wall. Ex: r23 through p45 hits enemy 78.
- **Discovered Attacks**: Move front piece, reveal back attack!
  Setup: Battery q behind b. Ex: b44 away → q attacks k!
- **Corridors**: Own file with double r—invade back rank.
  Setup: Trade p on file4, roll rs.
...
- **Vs Pins**: Warning: Slider lining file/diag to your VIP. Sac cheap piece, capture pinner (safe?), pawn block line.
- **Vs Skewers**: Warning: Slider on open file to your strong piece (junk behind). VIP capture attacker? Slide off line, block.
- **Vs X-Rays**: Warning: Enemy slider peeking through pawns. Move target, close file with trade.
- **Vs Discovered**: Warning: Enemy battery (q behind n), front moving. Block new line, king run, capture revealer.
- **Vs Corridors**: Warning: Enemy rs doubling file post-p-trade. Trade rs, counter-push p, t/n harass.
...
4. **Their hanging pieces** I can snag safe?
5. **Develops** (t/n/q out, p opens)?
6. Grabs **center** (44-55)?
7. Promos/10-move closing?
...
<!--CLOC-START -->
```cloc
Last updated at 2026-01-03 09:41:06 UTC
-------------------------------------------------------------------------------
Language                     files          blank        comment           code
-------------------------------------------------------------------------------
JSON                             3              0              0           2034
JavaScript                      10            402            319           1827
CSS                              2             85             13            452
Markdown                         3            136              2            407
HTML                             2              5              8            281
Text                             1             11              0             93
YAML                             2              8             13             37
-------------------------------------------------------------------------------
